...
JAR_EXISTS=$(find "services/certificate-service/target" -maxdepth 1 -name "*.jar" ! -name "*sources*" 2>/dev/null | head -n 1)
if [ "$BUILD_JAR" = true ] || [ -z "$JAR_EXISTS" ]; then
    echo "Compiling certificate-service with Maven..."
    if [ -f "./mvnw" ]; then
        ./mvnw package -DskipTests -pl "services/certificate-service" -am
    else
...
# 5. Build and Push
echo "Building certificate-service with Docker..."
...
# 6. Deploy or Update
APP_EXISTS=$(az containerapp list --resource-group "$RESOURCE_GROUP" --query "[?name=='certificate-service'].name" -o tsv)

if [ -z "$APP_EXISTS" ]; then
    echo "Creating Container App 'certificate-service'..."
    ...
    echo "Updating Container App 'certificate-service'..."
...
FQDN=$(az containerapp show --name "certificate-service" --resource-group "$RESOURCE_GROUP" --query "properties.configuration.ingress.fqdn" -o tsv 2>/dev/null || true)
echo "========================================================="
echo " CERTIFICATE-SERVICE DEPLOYED SUCCESSFULLY!"
if [ -n "$FQDN" ]; then
