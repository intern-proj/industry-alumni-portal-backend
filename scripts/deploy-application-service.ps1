# ==============================================================================
# Deploy application-service to Azure Container Apps
# Port: 8084 | Ingress: internal | CPU: 0.25 | Memory: 0.5Gi
# ==============================================================================
...
Write-Host " DEPLOYING: APPLICATION-SERVICE" -ForegroundColor Green
Write-Host " Registry:        $Registry.azurecr.io" -ForegroundColor Gray
Write-Host " Resource Group:  $ResourceGroup" -ForegroundColor Gray
Write-Host " Environment:     $Environment" -ForegroundColor Gray
Write-Host " Target Port:     8084 (Ingress: internal)" -ForegroundColor Gray
...
Write-Host "`n[4/5] Checking compiled JAR artifact for application-service..." -ForegroundColor Cyan
$JarExists = Get-ChildItem -Path "$ProjectRoot\services/application-service\target\*.jar" -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch "sources|original" }
if ($BuildJar -or (-not $JarExists)) {
    Write-Host "Building application-service with Maven..." -ForegroundColor Yellow
    if (Test-Path "$ProjectRoot\mvnw.cmd") {
        & "$ProjectRoot\mvnw.cmd" package -DskipTests -pl "services/application-service" -am
    } elseif (Get-Command mvn -ErrorAction SilentlyContinue) {
        mvn package -DskipTests -pl "services/application-service" -am
    } else {
        Write-Host "ERROR: Maven ('mvn') or Maven wrapper ('mvnw.cmd') not found." -ForegroundColor Red
        exit 1
    }
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Maven build failed for application-service." -ForegroundColor Red
        exit 1
    }
} else {
...
Write-Host "`n[5/5] Building application-service locally with Docker..." -ForegroundColor Cyan
docker build -t "$Registry.azurecr.io/application-service:$Version" -f "services/application-service/Dockerfile" .
if ($LASTEXITCODE -ne 0) {
    Write-Host "Docker build failed for application-service." -ForegroundColor Red
    exit 1
}
...
$PushOk = $false
for ($attempt = 1; $attempt -le 3; $attempt++) {
    docker push "$Registry.azurecr.io/application-service:$Version"
    if ($LASTEXITCODE -eq 0) {
        $PushOk = $true
        break
    }
...
Write-Host "`nDeploying Container App 'application-service' to Azure Container Apps..." -ForegroundColor Cyan
$AppExists = az containerapp list --resource-group $ResourceGroup --query "[?name=='application-service'].name" -o tsv

if (-not $AppExists) {
    Write-Host "Creating Container App 'application-service'..." -ForegroundColor Yellow
    az containerapp create `
        --name "application-service" `
        --resource-group $ResourceGroup `
        --environment $Environment `
        --image "$Registry.azurecr.io/application-service:$Version" `
    ...
    Write-Host "Updating Container App 'application-service'..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Failed to deploy application-service." -ForegroundColor Red
    exit 1
}

$Fqdn = az containerapp show --name "application-service" --resource-group $ResourceGroup --query "properties.configuration.ingress.fqdn" -o tsv 2>$null

Write-Host "`n=========================================================" -ForegroundColor Green
Write-Host " APPLICATION-SERVICE DEPLOYED SUCCESSFULLY!" -ForegroundColor Green
