...
echo "========================================================="
echo " DEPLOYING: AUDIT-STORAGE-SERVICE"
echo "========================================================="
...
JAR_EXISTS=$(find "services/audit-storage-service/target" -maxdepth 1 -name "*.jar" ! -name "*sources*" 2>/dev/null | head -n 1)
if [ "$BUILD_JAR" = true ] || [ -z "$JAR_EXISTS" ]; then
    echo "Compiling audit-storage-service with Maven..."
    if [ -f "./mvnw" ]; then
        ./mvnw package -DskipTests -pl "services/audit-storage-service" -am
    else
...
# 5. Build and Push
echo "Building audit-storage-service with Docker..."
...
# 6. Deploy or Update
APP_EXISTS=$(az containerapp list --resource-group "$RESOURCE_GROUP" --query "[?name=='audit-storage-service'].name" -o tsv)

if [ -z "$APP_EXISTS" ]; then
    echo "Creating Container App 'audit-storage-service'..."
    ...
    echo "Updating Container App 'audit-storage-service'..."
...
FQDN=$(az containerapp show --name "audit-storage-service" --resource-group "$RESOURCE_GROUP" --query "properties.configuration.ingress.fqdn" -o tsv 2>/dev/null || true)
echo "========================================================="
echo " AUDIT-STORAGE-SERVICE DEPLOYED SUCCESSFULLY!"
if [ -n "$FQDN" ]; then
