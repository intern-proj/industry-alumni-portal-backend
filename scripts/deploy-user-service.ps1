...
# Port: 8090 | Ingress: internal | CPU: 0.25 | Memory: 0.5Gi
# ==============================================================================
...
Write-Host " Target Port:     8090 (Ingress: internal)" -ForegroundColor Gray
...
Write-Host "`n[4/5] Checking compiled JAR artifact for user-service..." -ForegroundColor Cyan
$JarExists = Get-ChildItem -Path "$ProjectRoot\services/user-service\target\*.jar" -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch "sources|original" }
if ($BuildJar -or (-not $JarExists)) {
    Write-Host "Building user-service with Maven..." -ForegroundColor Yellow
    if (Test-Path "$ProjectRoot\mvnw.cmd") {
        & "$ProjectRoot\mvnw.cmd" package -DskipTests -pl "services/user-service" -am
    } elseif (Get-Command mvn -ErrorAction SilentlyContinue) {
        mvn package -DskipTests -pl "services/user-service" -am
    } else {
        Write-Host "ERROR: Maven ('mvn') or Maven wrapper ('mvnw.cmd') not found." -ForegroundColor Red
        exit 1
    }
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Maven build failed for user-service." -ForegroundColor Red
        exit 1
    }
} else {
...
Write-Host "`n[5/5] Building user-service locally with Docker..." -ForegroundColor Cyan
docker build -t "$Registry.azurecr.io/user-service:$Version" -f "services/user-service/Dockerfile" .
if ($LASTEXITCODE -ne 0) {
    Write-Host "Docker build failed for user-service." -ForegroundColor Red
    exit 1
}
...
$PushOk = $false
for ($attempt = 1; $attempt -le 3; $attempt++) {
    docker push "$Registry.azurecr.io/user-service:$Version"
    if ($LASTEXITCODE -eq 0) {
        $PushOk = $true
        break
    }
...
Write-Host "`nDeploying Container App 'user-service' to Azure Container Apps..." -ForegroundColor Cyan
$AppExists = az containerapp list --resource-group $ResourceGroup --query "[?name=='user-service'].name" -o tsv

if (-not $AppExists) {
    Write-Host "Creating Container App 'user-service'..." -ForegroundColor Yellow
    az containerapp create `
        --name "user-service" `
        --resource-group $ResourceGroup `
        --environment $Environment `
        --image "$Registry.azurecr.io/user-service:$Version" `
    ...
    Write-Host "Updating Container App 'user-service'..." -ForegroundColor Yellow
...
$Fqdn = az containerapp show --name "user-service" --resource-group $ResourceGroup --query "properties.configuration.ingress.fqdn" -o tsv 2>$null

Write-Host "`n=========================================================" -ForegroundColor Green
Write-Host " USER-SERVICE DEPLOYED SUCCESSFULLY!" -ForegroundColor Green
