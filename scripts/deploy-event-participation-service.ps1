...
# Port: 8083 | Ingress: internal | CPU: 0.25 | Memory: 0.5Gi
# ==============================================================================
...
Write-Host " DEPLOYING: EVENT-PARTICIPATION-SERVICE" -ForegroundColor Green
Write-Host " Registry:        $Registry.azurecr.io" -ForegroundColor Gray
Write-Host " Resource Group:  $ResourceGroup" -ForegroundColor Gray
Write-Host " Environment:     $Environment" -ForegroundColor Gray
Write-Host " Target Port:     8083 (Ingress: internal)" -ForegroundColor Gray
...
Write-Host "`n[4/5] Checking compiled JAR artifact for event-participation-service..." -ForegroundColor Cyan
$JarExists = Get-ChildItem -Path "$ProjectRoot\services/event-participation-service\target\*.jar" -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch "sources|original" }
if ($BuildJar -or (-not $JarExists)) {
    Write-Host "Building event-participation-service with Maven..." -ForegroundColor Yellow
    if (Test-Path "$ProjectRoot\mvnw.cmd") {
        & "$ProjectRoot\mvnw.cmd" package -DskipTests -pl "services/event-participation-service" -am
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Maven build failed for event-participation-service." -ForegroundColor Red
        exit 1
    }
} else {
...
Write-Host "`n[5/5] Building event-participation-service locally with Docker..." -ForegroundColor Cyan
docker build -t "$Registry.azurecr.io/event-participation-service:$Version" -f "services/event-participation-service/Dockerfile" .
if ($LASTEXITCODE -ne 0) {
    Write-Host "Docker build failed for event-participation-service." -ForegroundColor Red
    exit 1
}
...
Write-Host "`nDeploying Container App 'event-participation-service' to Azure Container Apps..." -ForegroundColor Cyan
$AppExists = az containerapp list --resource-group $ResourceGroup --query "[?name=='event-participation-service'].name" -o tsv

if (-not $AppExists) {
    Write-Host "Creating Container App 'event-participation-service'..." -ForegroundColor Yellow
    ...
    Write-Host "Updating Container App 'event-participation-service'..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Failed to deploy event-participation-service." -ForegroundColor Red
    exit 1
}

$Fqdn = az containerapp show --name "event-participation-service" --resource-group $ResourceGroup --query "properties.configuration.ingress.fqdn" -o tsv 2>$null

Write-Host "`n=========================================================" -ForegroundColor Green
Write-Host " EVENT-PARTICIPATION-SERVICE DEPLOYED SUCCESSFULLY!" -ForegroundColor Green
