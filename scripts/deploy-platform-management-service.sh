...
echo "========================================================="
echo " DEPLOYING: PLATFORM-MANAGEMENT-SERVICE"
echo "========================================================="
...
JAR_EXISTS=$(find "services/platform-management-service/target" -maxdepth 1 -name "*.jar" ! -name "*sources*" 2>/dev/null | head -n 1)
if [ "$BUILD_JAR" = true ] || [ -z "$JAR_EXISTS" ]; then
    echo "Compiling platform-management-service with Maven..."
    if [ -f "./mvnw" ]; then
        ./mvnw package -DskipTests -pl "services/platform-management-service" -am
    else
...
# 5. Build and Push
echo "Building platform-management-service with Docker..."
...
# 6. Deploy or Update
APP_EXISTS=$(az containerapp list --resource-group "$RESOURCE_GROUP" --query "[?name=='platform-management-service'].name" -o tsv)

if [ -z "$APP_EXISTS" ]; then
    echo "Creating Container App 'platform-management-service'..."
    ...
    echo "Updating Container App 'platform-management-service'..."
...
FQDN=$(az containerapp show --name "platform-management-service" --resource-group "$RESOURCE_GROUP" --query "properties.configuration.ingress.fqdn" -o tsv 2>/dev/null || true)
echo "========================================================="
echo " PLATFORM-MANAGEMENT-SERVICE DEPLOYED SUCCESSFULLY!"
if [ -n "$FQDN" ]; then
