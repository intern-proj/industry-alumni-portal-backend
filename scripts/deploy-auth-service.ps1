# ==============================================================================
# Deploy auth-service to Azure Container Apps
# Port: 8081 | Ingress: internal | CPU: 0.25 | Memory: 0.5Gi
# ==============================================================================
...
Write-Host " Target Port:     8081 (Ingress: internal)" -ForegroundColor Gray
...
Write-Host "`n[4/5] Checking compiled JAR artifact for auth-service..." -ForegroundColor Cyan
$JarExists = Get-ChildItem -Path "$ProjectRoot\services/auth-service\target\*.jar" -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch "sources|original" }
if ($BuildJar -or (-not $JarExists)) {
    Write-Host "Building auth-service with Maven..." -ForegroundColor Yellow
    if (Test-Path "$ProjectRoot\mvnw.cmd") {
        & "$ProjectRoot\mvnw.cmd" package -DskipTests -pl "services/auth-service" -am
    } elseif (Get-Command mvn -ErrorAction SilentlyContinue) {
        mvn package -DskipTests -pl "services/auth-service" -am
    } else {
        Write-Host "ERROR: Maven ('mvn') or Maven wrapper ('mvnw.cmd') not found." -ForegroundColor Red
        exit 1
    }
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Maven build failed for auth-service." -ForegroundColor Red
        exit 1
    }
} else {
...
Write-Host "`n[5/5] Building auth-service locally with Docker..." -ForegroundColor Cyan
docker build -t "$Registry.azurecr.io/auth-service:$Version" -f "services/auth-service/Dockerfile" .
if ($LASTEXITCODE -ne 0) {
    Write-Host "Docker build failed for auth-service." -ForegroundColor Red
    exit 1
}
...
$PushOk = $false
for ($attempt = 1; $attempt -le 3; $attempt++) {
    docker push "$Registry.azurecr.io/auth-service:$Version"
    if ($LASTEXITCODE -eq 0) {
        $PushOk = $true
        break
    }
...
Write-Host "`nDeploying Container App 'auth-service' to Azure Container Apps..." -ForegroundColor Cyan
$AppExists = az containerapp list --resource-group $ResourceGroup --query "[?name=='auth-service'].name" -o tsv

if (-not $AppExists) {
    Write-Host "Creating Container App 'auth-service'..." -ForegroundColor Yellow
    az containerapp create `
        --name "auth-service" `
        --resource-group $ResourceGroup `
        --environment $Environment `
        --image "$Registry.azurecr.io/auth-service:$Version" `
    ...
    Write-Host "Updating Container App 'auth-service'..." -ForegroundColor Yellow
...
$Fqdn = az containerapp show --name "auth-service" --resource-group $ResourceGroup --query "properties.configuration.ingress.fqdn" -o tsv 2>$null
