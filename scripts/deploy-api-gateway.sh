...
JAR_EXISTS=$(find "api-gateway/target" -maxdepth 1 -name "*.jar" ! -name "*sources*" 2>/dev/null | head -n 1)
if [ "$BUILD_JAR" = true ] || [ -z "$JAR_EXISTS" ]; then
    echo "Compiling api-gateway with Maven..."
    if [ -f "./mvnw" ]; then
        ./mvnw package -DskipTests -pl "api-gateway" -am
    else
...
# 6. Deploy or Update
APP_EXISTS=$(az containerapp list --resource-group "$RESOURCE_GROUP" --query "[?name=='api-gateway'].name" -o tsv)

if [ -z "$APP_EXISTS" ]; then
    echo "Creating Container App 'api-gateway'..."
    ...
    echo "Updating Container App 'api-gateway'..."
...
FQDN=$(az containerapp show --name "api-gateway" --resource-group "$RESOURCE_GROUP" --query "properties.configuration.ingress.fqdn" -o tsv 2>/dev/null || true)
echo "========================================================="
