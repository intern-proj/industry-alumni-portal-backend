...
JAR_EXISTS=$(find "services/application-service/target" -maxdepth 1 -name "*.jar" ! -name "*sources*" 2>/dev/null | head -n 1)
if [ "$BUILD_JAR" = true ] || [ -z "$JAR_EXISTS" ]; then
    echo "Compiling application-service with Maven..."
    if [ -f "./mvnw" ]; then
        ./mvnw package -DskipTests -pl "services/application-service" -am
    else
...
# 5. Build and Push
echo "Building application-service with Docker..."
...
# 6. Deploy or Update
APP_EXISTS=$(az containerapp list --resource-group "$RESOURCE_GROUP" --query "[?name=='application-service'].name" -o tsv)

if [ -z "$APP_EXISTS" ]; then
    echo "Creating Container App 'application-service'..."
    ...
    echo "Updating Container App 'application-service'..."
...
FQDN=$(az containerapp show --name "application-service" --resource-group "$RESOURCE_GROUP" --query "properties.configuration.ingress.fqdn" -o tsv 2>/dev/null || true)
echo "========================================================="
