...
echo "========================================================="
echo " DEPLOYING: EVENT-MANAGEMENT-SERVICE"
echo "========================================================="
...
JAR_EXISTS=$(find "services/event-management-service/target" -maxdepth 1 -name "*.jar" ! -name "*sources*" 2>/dev/null | head -n 1)
if [ "$BUILD_JAR" = true ] || [ -z "$JAR_EXISTS" ]; then
    echo "Compiling event-management-service with Maven..."
    if [ -f "./mvnw" ]; then
        ./mvnw package -DskipTests -pl "services/event-management-service" -am
    else
...
# 5. Build and Push
echo "Building event-management-service with Docker..."
...
# 6. Deploy or Update
APP_EXISTS=$(az containerapp list --resource-group "$RESOURCE_GROUP" --query "[?name=='event-management-service'].name" -o tsv)

if [ -z "$APP_EXISTS" ]; then
    echo "Creating Container App 'event-management-service'..."
    ...
    echo "Updating Container App 'event-management-service'..."
...
FQDN=$(az containerapp show --name "event-management-service" --resource-group "$RESOURCE_GROUP" --query "properties.configuration.ingress.fqdn" -o tsv 2>/dev/null || true)
echo "========================================================="
echo " EVENT-MANAGEMENT-SERVICE DEPLOYED SUCCESSFULLY!"
if [ -n "$FQDN" ]; then
