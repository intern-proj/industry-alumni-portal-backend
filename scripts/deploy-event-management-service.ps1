# ==============================================================================
# Deploy event-management-service to Azure Container Apps
# Port: 8082 | Ingress: internal | CPU: 0.25 | Memory: 0.5Gi
# ==============================================================================
...
Write-Host " DEPLOYING: EVENT-MANAGEMENT-SERVICE" -ForegroundColor Green
Write-Host " Registry:        $Registry.azurecr.io" -ForegroundColor Gray
Write-Host " Resource Group:  $ResourceGroup" -ForegroundColor Gray
Write-Host " Environment:     $Environment" -ForegroundColor Gray
Write-Host " Target Port:     8082 (Ingress: internal)" -ForegroundColor Gray
...
Write-Host "`n[4/5] Checking compiled JAR artifact for event-management-service..." -ForegroundColor Cyan
$JarExists = Get-ChildItem -Path "$ProjectRoot\services/event-management-service\target\*.jar" -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch "sources|original" }
if ($BuildJar -or (-not $JarExists)) {
    Write-Host "Building event-management-service with Maven..." -ForegroundColor Yellow
    if (Test-Path "$ProjectRoot\mvnw.cmd") {
        & "$ProjectRoot\mvnw.cmd" package -DskipTests -pl "services/event-management-service" -am
    } elseif (Get-Command mvn -ErrorAction SilentlyContinue) {
        mvn package -DskipTests -pl "services/event-management-service" -am
    } else {
        Write-Host "ERROR: Maven ('mvn') or Maven wrapper ('mvnw.cmd') not found." -ForegroundColor Red
        exit 1
    }
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Maven build failed for event-management-service." -ForegroundColor Red
        exit 1
    }
} else {
...
Write-Host "`n[5/5] Building event-management-service locally with Docker..." -ForegroundColor Cyan
docker build -t "$Registry.azurecr.io/event-management-service:$Version" -f "services/event-management-service/Dockerfile" .
if ($LASTEXITCODE -ne 0) {
    Write-Host "Docker build failed for event-management-service." -ForegroundColor Red
    exit 1
}
...
$PushOk = $false
for ($attempt = 1; $attempt -le 3; $attempt++) {
    docker push "$Registry.azurecr.io/event-management-service:$Version"
    if ($LASTEXITCODE -eq 0) {
        $PushOk = $true
        break
    }
...
Write-Host "`nDeploying Container App 'event-management-service' to Azure Container Apps..." -ForegroundColor Cyan
$AppExists = az containerapp list --resource-group $ResourceGroup --query "[?name=='event-management-service'].name" -o tsv

if (-not $AppExists) {
    Write-Host "Creating Container App 'event-management-service'..." -ForegroundColor Yellow
    az containerapp create `
        --name "event-management-service" `
        --resource-group $ResourceGroup `
        --environment $Environment `
        --image "$Registry.azurecr.io/event-management-service:$Version" `
    ...
    Write-Host "Updating Container App 'event-management-service'..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Failed to deploy event-management-service." -ForegroundColor Red
    exit 1
}

$Fqdn = az containerapp show --name "event-management-service" --resource-group $ResourceGroup --query "properties.configuration.ingress.fqdn" -o tsv 2>$null

Write-Host "`n=========================================================" -ForegroundColor Green
Write-Host " EVENT-MANAGEMENT-SERVICE DEPLOYED SUCCESSFULLY!" -ForegroundColor Green
