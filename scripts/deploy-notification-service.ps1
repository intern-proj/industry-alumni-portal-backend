...
# Port: 8088 | Ingress: internal | CPU: 0.25 | Memory: 0.5Gi
# ==============================================================================
...
Write-Host " DEPLOYING: NOTIFICATION-SERVICE" -ForegroundColor Green
Write-Host " Registry:        $Registry.azurecr.io" -ForegroundColor Gray
Write-Host " Resource Group:  $ResourceGroup" -ForegroundColor Gray
Write-Host " Environment:     $Environment" -ForegroundColor Gray
Write-Host " Target Port:     8088 (Ingress: internal)" -ForegroundColor Gray
...
Write-Host "`n[4/5] Checking compiled JAR artifact for notification-service..." -ForegroundColor Cyan
$JarExists = Get-ChildItem -Path "$ProjectRoot\services/notification-service\target\*.jar" -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch "sources|original" }
if ($BuildJar -or (-not $JarExists)) {
    Write-Host "Building notification-service with Maven..." -ForegroundColor Yellow
    if (Test-Path "$ProjectRoot\mvnw.cmd") {
        & "$ProjectRoot\mvnw.cmd" package -DskipTests -pl "services/notification-service" -am
    } elseif (Get-Command mvn -ErrorAction SilentlyContinue) {
        mvn package -DskipTests -pl "services/notification-service" -am
    } else {
        Write-Host "ERROR: Maven ('mvn') or Maven wrapper ('mvnw.cmd') not found." -ForegroundColor Red
        exit 1
    }
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Maven build failed for notification-service." -ForegroundColor Red
        exit 1
    }
} else {
...
Write-Host "`n[5/5] Building notification-service locally with Docker..." -ForegroundColor Cyan
docker build -t "$Registry.azurecr.io/notification-service:$Version" -f "services/notification-service/Dockerfile" .
if ($LASTEXITCODE -ne 0) {
    Write-Host "Docker build failed for notification-service." -ForegroundColor Red
    exit 1
}
...
$PushOk = $false
for ($attempt = 1; $attempt -le 3; $attempt++) {
    docker push "$Registry.azurecr.io/notification-service:$Version"
    if ($LASTEXITCODE -eq 0) {
        $PushOk = $true
        break
    }
...
Write-Host "`nDeploying Container App 'notification-service' to Azure Container Apps..." -ForegroundColor Cyan
$AppExists = az containerapp list --resource-group $ResourceGroup --query "[?name=='notification-service'].name" -o tsv

if (-not $AppExists) {
    Write-Host "Creating Container App 'notification-service'..." -ForegroundColor Yellow
    az containerapp create `
        --name "notification-service" `
        --resource-group $ResourceGroup `
        --environment $Environment `
        --image "$Registry.azurecr.io/notification-service:$Version" `
    ...
    Write-Host "Updating Container App 'notification-service'..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Failed to deploy notification-service." -ForegroundColor Red
    exit 1
}

$Fqdn = az containerapp show --name "notification-service" --resource-group $ResourceGroup --query "properties.configuration.ingress.fqdn" -o tsv 2>$null

Write-Host "`n=========================================================" -ForegroundColor Green
Write-Host " NOTIFICATION-SERVICE DEPLOYED SUCCESSFULLY!" -ForegroundColor Green
